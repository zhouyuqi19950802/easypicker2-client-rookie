...
// 强制轮询的定时器 ID
let forcePollTimer: any = null
// 已下载的任务ID集合（避免重复下载）
const downloadedTaskIds = new Set<string>(
  JSON.parse(localStorage.getItem('ep_downloaded_tasks') || '[]')
)

// 保存已下载任务列表并清理旧的记录（只保留最近50个）
function saveDownloadedTasks() {
  // 如果超过50个，只保留最近50个
  if (downloadedTaskIds.size > 50) {
    const idsArray = Array.from(downloadedTaskIds)
    const recentIds = idsArray.slice(-50) // 保留最近50个
    downloadedTaskIds.clear()
    recentIds.forEach(id => downloadedTaskIds.add(id))
  }
  // 保存到 localStorage
  localStorage.setItem('ep_downloaded_tasks', JSON.stringify(Array.from(downloadedTaskIds)))
}
  ...
  ActionServiceAPI.getDownloadActions(
    ...
    actions
      ...
      .forEach((action) => {
        ...
        // 判断状态
        // SUCCESS
        //  如果任务在 compressTask 中，或者任务刚完成且不在已下载列表中，触发下载
        if (action.status === DownloadStatus.SUCCESS && action.url) {
          // 检查是否已经下载过
          if (downloadedTaskIds.has(action.id)) {
            // 已经下载过，跳过
            return
          }
          ...
          if (existIndex !== -1) {
            ...
            // 标记为已下载
            downloadedTaskIds.add(action.id)
            saveDownloadedTasks()
          } else {
            // 任务不在跟踪列表中，但状态是 SUCCESS，可能是刚完成的，也触发下载
            // 检查是否已经在已下载的历史记录中（通过日期判断，如果是最近1分钟内的任务，可能是新完成的）
            const taskDate = new Date(action.date).getTime()
            const now = Date.now()
            const isRecentTask = (now - taskDate) < 60000 // 1分钟内
            if (isRecentTask) {
              // 是新完成的任务，自动下载
              downloadUrl.value = action.url
              showLinkModel.value = true
  ...
  switch (e) {
    case 'download':
      ...
      FileApi.batchDownload(
        ...
        .then(() => {
          // 批量下载后立即开始轮询，确保能捕获到新创建的任务
          loadActions()
          // 清除之前的强制轮询定时器
          if (forcePollTimer) {
            clearInterval(forcePollTimer)
          }
          // 启动强制轮询，确保即使任务很快完成也能被捕获
          let pollCount = 0
  ...
  <div class="files">
    ...
    <div class="panel">
      ...
      >
        ...
        <el-table-column prop="date" label="提交时间" width="180" align="center" header-align="center">
          <template #default="scope">
            {{
              formatDate(new Date(scope.row.date))
            }}
        ...
        <el-table-column prop="task_name" label="任务" width="150" align="center" header-align="center" />
        <el-table-column prop="name" label="文件名" width="160" align="center" header-align="center" />
        <template v-if="showOriginName">
          <el-table-column prop="origin_name" label="原文件名" width="200" align="center" header-align="center">
            <template #default="scope">
              {{ scope.row.origin_name || '-' }}
            </template>
          </el-table-column>
        </template>
        <el-table-column prop="size" label="大小" width="120" align="center" header-align="center">
          <template #default="scope">
            {{
        ...
        <el-table-column prop="downloadCount" width="90" label="下载次数" align="center" header-align="center" />
        <template v-if="showImg">
          <el-table-column label="缩略图" width="120" align="center" header-align="center">
            <template #default="scope">
              <el-image
        ...
        <template v-if="showPeople">
          <el-table-column prop="people" width="90" label="限制名单" align="center" header-align="center">
            <template #default="scope">
              {{ scope.row.people || '-' }}
            </template>
          </el-table-column>
        </template>
        <el-table-column fixed="right" label="操作" width="140" align="center" header-align="center">
          <template #default="scope">
            <div class="text-btns">
