  ...
  return copyUsers
    ...
    .filter((v) => {
      ...
      if (searchWord.value.length === 0)
        return true
      return `${id} ${account} ${email || ''} ${loginTime} ${formatDate(
        openTime,
...
function getCode() {
  if (!rEmail.test(emailForm.email)) {
    ElMessage.warning('邮箱格式不正确')
    return
  }
  // check是否可用
  PublicApi.checkEmail(emailForm.email)
    .then(() => {
      PublicApi.getCode(undefined, 'default').then(() => {
        time.value = 120
        refreshCodeText()
        ElMessage.success('验证码已发送至您的登录邮箱，请注意查收')
      })
    })
    .catch((err) => {
      const { code: c } = err
...
function checkEmailForm() {
  ...
  if (!rVerCode.test(emailForm.code)) {
...
function handleBindEmail(id: number) {
  selectUserId.value = id
...
async function handleSaveEmail() {
  ...
  // 调用API更新，验证码 不正确判断
  SuperUserApi.resetEmail(selectUserId.value, emailForm.email, emailForm.code)
    .then(() => {
          ...
          <template #default="scope">
            <div class="text-btn-list">
              ...
              <el-button
                ...
                text
                size="small"
                @click="handleBindEmail(scope.row.id)"
              >
                绑定邮箱
              </el-button>
              <el-button
