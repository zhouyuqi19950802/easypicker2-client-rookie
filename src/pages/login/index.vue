  ...
  <div class="login">
    <login-panel>
      ...
      <div class="inputArea">
        <div>
          <el-input
            :placeholder="accountLogin ? '输入账号/邮箱' : '输入邮箱'"
            :prefix-icon="accountLogin ? User : Message"
            v-model="account"
            clearable
          >
            <template #append>
        ...
        <div>
          <el-input
            maxlength="16"
            minlength="6"
            :type="accountLogin ? 'password' : 'text'"
            :inputmode="accountLogin ? undefined : 'numeric'"
...
const checkForm = () => {
  if (!accountLogin.value && !rEmail.test(account.value)) {
    ElMessage.warning('邮箱格式不正确')
    return false
  }

  if (accountLogin.value && !rPassword.test(pwd.value)) {
    ElMessage.warning('密码格式不正确(6-16位 支持字母/数字/下划线/连字符)')
    return false
  }
...
const getCode = () => {
  if (!rEmail.test(account.value)) {
    ElMessage.warning('邮箱格式不正确')
    return
  }
  PublicApi.getCode(account.value, 'login')
    .then(() => {
      time.value = 120
      refreshCodeText()
      ElMessage.success('获取成功,请注意查收邮箱')
    })
    .catch((err) => {
      const { code: c } = err
      const msg = '获取验证码失败,未知错误'
      const options = {
        1006: '邮箱格式不正确',
        1008: '该邮箱未绑定任何账号，请检查后重试'
      }
